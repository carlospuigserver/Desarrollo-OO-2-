...
class GUI(tk.Tk):
    def __init__(self):
        ...
        self.button_ingresar = tk.Button(self, text="Ingresar", command=self.iniciar_sesion)

        self.label_menu = tk.Label(self, text="¿Qué menú desea ver?")
        self.button_menu_simple = tk.Button(self, text="Menú Simple", command=lambda: self.ejecutar_pedido(0))
        self.button_menu_ninos = tk.Button(self, text="Menú Niños", command=lambda: self.ejecutar_pedido(1))
        self.button_menu_halloween = tk.Button(self, text="Menú Halloween", command=lambda: self.ejecutar_pedido(2))
        self.button_menu_san_valentin = tk.Button(self, text="Menú San Valentín", command=lambda: self.ejecutar_pedido(3))
        self.button_menu_combo_pareja = tk.Button(self, text="Menú Combo Pareja", command=lambda: self.ejecutar_pedido(4))

        self.label_usuario.pack(pady=10)
        self.entry_usuario.pack(pady=5)
        self.label_contraseña.pack(pady=10)
        self.entry_contraseña.pack(pady=5)
        self.button_ingresar.pack(pady=20)
    ...
    def iniciar_sesion(self):
        ...
        if autenticar_usuario(nombre_usuario, contraseña):
            ...
            self.deiconify()  # Mostrar la ventana principal después de iniciar sesión
